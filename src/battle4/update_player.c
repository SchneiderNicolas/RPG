...
int update_player_turn_4(gui_t *game)
{
    static int i = 0;
    ...
    if (i == 20) {
        ...
        move_rect(game->object[ENEMY_IDLE_4]);
        i = 0;
    } else {
        i++;
    }
    ...
    sfSprite_setTextureRect(game->object[ENEMY_IDLE_4]->sprite,
    game->object[ENEMY_IDLE_4]->rect);
    return (1);
}

int update_player_attack4_2(gui_t *game)
{
    static int i = 0;
    static int j = 0;
    ...
    if (i == 20 && j < 3) {
        ...
        move_rect(game->object[ENEMY_HURT_4]);
        i = 0;
    } else
        i++;
    ...
    game->object[ARMOR1_ATTACK]->rect.width >= max_value && i == 0)
        j++;
    if (j == 3) {
        game->battle4->status = REMOVE_ENEMY_HP;
        game->battle4->hp -= game->player->next_attack;
...
int update_player_attack_4(gui_t *game)
{
    update_player_attack4_2(game);
    ...
    sfSprite_setTextureRect(game->object[ENEMY_HURT_4]->sprite,
    game->object[ENEMY_HURT_4]->rect);
    return (1);
}

int update_player_boost4_2(gui_t *game)
{
...
int update_player_boost_4(gui_t *game)
{
    update_player_boost4_2(game);
