...
int update_player_turn_3(gui_t *game)
{
    static int i = 0;

    if (i == 20) {
        move_rect(game->object[ARMOR1_IDLE]);
        move_rect(game->object[ENEMY_IDLE_3]);
        i = 0;
    } else {
        i++;
    }
    ...
    sfSprite_setTextureRect(game->object[ENEMY_IDLE_3]->sprite,
    game->object[ENEMY_IDLE_3]->rect);
    return (1);
}

int update_player_attack3_2(gui_t *game)
{
    static int i = 0;
    static int j = 0;
    ...
    if (i == 20 && j < 3) {
        ...
        move_rect(game->object[ENEMY_HURT_3]);
        i = 0;
    } else
        i++;
    ...
    game->object[ARMOR1_ATTACK]->rect.width >= max_value && i == 0)
        j++;
    if (j == 3) {
        game->battle3->status = REMOVE_ENEMY_HP;
        game->battle3->hp -= game->player->next_attack;
...
int update_player_attack_3(gui_t *game)
{
    update_player_attack3_2(game);
    sfSprite_setTextureRect(game->object[ARMOR1_ATTACK]->sprite,
    game->object[ARMOR1_ATTACK]->rect);
    sfSprite_setTextureRect(game->object[ENEMY_HURT_3]->sprite,
    game->object[ENEMY_HURT_3]->rect);
    return (1);
}

int update_player_boost3_2(gui_t *game)
{
    static int i = 0;
    static int j = 0;
    unsigned int max_value =
    ...
    game->object[ARMOR1_BOOST]->rect.width
    >= max_value && i == 0)
        j++;
    if (j == 3) {
        game->battle3->status = ENEMY_ATTACK;
        game->player->next_attack += 70;
        game->object[ARMOR1_BOOST]->rect.left = 0;
        j = 0;
    }
}

int update_player_boost_3(gui_t *game)
{
    update_player_boost3_2(game);
